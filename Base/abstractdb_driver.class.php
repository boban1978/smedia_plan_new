...
class AbstractDB_Driver 
{
   ...
   function InitSp($spName){
		die("Method AbstractDB_Driver::InitSp() must be redefined in extended classes without calling parent.");
   	   }

   function ExecuteSp($sp){
		die("Method AbstractDB_Driver::ExecuteSp() must be redefined in extended classes without calling parent.");
   	   }
	/**
	...
	*/
	...
	/**
	 * Izvrsava STORE PROCEDURE
	 * @access public
	 * @internal This method must be overriden in extended classes with a call to parent if this feature is supported.
	 * @param string $spName Ime Store Procedure
	 * @param string $parameters Niz parametra sa kojima se poziva STORE procedura
	 * 
	 */
   function SP_CALL($spName,$parameters,$outputType="Message"){//CALL Stored Procedure
   			die("Method AbstractDB_Driver::SP_CALL() must be redefined in extended classes without calling parent.");	
   }	
	/**
