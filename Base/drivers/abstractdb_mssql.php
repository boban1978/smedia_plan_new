<?php
/**
* AbstractDB MSSQLDriver
* Radio i prepisao Nikola LEkic
* 
* @package AbstractDB
* @author Pacific-Cybersoft, Nikola Lekic
...
/**
* M Driver included constant.
* 
* Flag that indicates that the MSSQL driver has been included.
* @access public
*/
define("ABSTRACTDB_MSSQL_INCLUDED", false);
...
* @package AbstractDB
*/
class AbstractDB_MSSQL extends AbstractDB_Driver
{
	/**
	...
	*/
	function AbstractDB_MSSQL($arguments)
	{
	...
	function DataSeek(&$rs, $row_num)
	{
		...
		return @mssql_data_seek($rs, $row_num);
	}
	...
	function FieldNames(&$rs, &$fields)
	{
		...
		{
			$fields[] = @mssql_field_name($rs, $index);
...
/*	function InsertID()
	...
	function Query($sql)
	{
		if(!$this->Connect())
			return false;
		if(!(@mssql_select_db($this->_arguments["Database"], $this->_connection) && $rs = @mssql_query($sql, $this->_connection)))
		{
			$this->SetError("Query", mssql_get_last_message());
			return false;
		}
		return $rs;
	}
	
   function InitSp($spName){
   		if(!$this->Connect())
			return false;
		if(!(@mssql_select_db($this->_arguments["Database"], $this->_connection) && $rs = @mssql_init($spName,$this->_connection)))
		{
			$this->SetError("Query", mssql_get_last_message());
			return false;
		}
   ...
   function ExecuteSp($sp){
   	   	if(!$this->Connect())
			return false;
		if(!(@mssql_select_db($this->_arguments["Database"], $this->_connection) && $rs = @mssql_execute($sp)))
		{
			$this->SetError("Query", mssql_get_last_message());
			return false;
		}
   ...
   //Ovo treba probati da se popravi
    function SP_EXECUTE($spName, $parameters,$outputType="Message"){//CALL Stored Procedure
   	var_dump($parameters);
   		if(!$this->Connect())
		return false;
   		$sps=mssql_init($spName,$this->_connection);
   		$z=0;
   		$outputparams=array();
   		//var_dump($parameters[0]["parameter"]);
	...
	function SetDatabase($dbName)
	{
		if(!$this->Connect())
			return false;
		if(!($result = @mssql_select_db($dbName, $this->_connection)))
			$this->SetError("SetDatabase", mssql_get_last_message());
		else
		{
	...
	function Connect()
	{
	    ...
	    if(isset($this->_arguments["Options"]["Persistent"]) && 
			...
			function_exists("mssql_pconnect"))
		...
		if(!$this->_connection)
		{
		    $this->SetError("Connect", isset($php_errormsg) ? $php_errormsg : "Could not connect to MSSQL server");
		    return false;
		}
		return true;
	}
