...
class AbstractDB
{
	...
	* @var array
	*/
	var $_arguments = array("Type" => DB_TYPE, "Username" => USERNAME, "Password" => PASSWORD, "Hostname" => HOSTNAME, "Database" => DBNAME, "Options" => array("Persistent" => 1));
	/**
	...
	*/
	...
	{
		$this->ClearError();
		$result = &$this->_driver->InitSp($spName);
		$this->GetDriverError();
		return $result;
	}	
	...
	{
		$this->ClearError();
		$result = &$this->_driver->ExecuteSp($sp);
		$this->GetDriverError();
		return $result;
	}
		/**
	 ...
	 * 
	 */
   function SP_EXECUTE($spName,$parameters=Array(),$outputType="Message"){//CALL Stored Procedure
		$this->ClearError();
		$result = &$this->_driver->SP_EXECUTE($spName,$parameters=Array(),$outputType="Message");
		$this->GetDriverError();
		return $result;   }
