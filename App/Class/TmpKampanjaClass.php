        ...
        foreach ($daysForCampaign as $availableDay) {
            ...
            */
            ...
            foreach ($rows['rows'] as $row) {
                ...
                $key_pom=substr('000'.$preostalo_sekundi,-3)."_".$row['BlokID'];

                switch (1) {
                    case $sat >= 6 && $sat <= 7:
                        $freeBlocksForDays[1][$availableDay][$key_pom]=$row['BlokID'];
                        //array_push($freeBlocksForDays[1][$availableDay], $row['BlokID']);
                        break;
                    case $sat >= 8 && $sat <= 11:
                        //$preostalo_sekundi=rand(100,300);
                        $freeBlocksForDays[2][$availableDay][$key_pom]=$row['BlokID'];
                        //array_push($freeBlocksForDays[2][$availableDay], $row['BlokID']);
                        break;
                    case $sat >= 12 && $sat <= 16:
                        $freeBlocksForDays[3][$availableDay][$key_pom]=$row['BlokID'];
                        //array_push($freeBlocksForDays[3][$availableDay], $row['BlokID']);
                        break;
                    case $sat >= 17 && $sat <= 19:
                        $freeBlocksForDays[4][$availableDay][$key_pom]=$row['BlokID'];
                        //array_push($freeBlocksForDays[4][$availableDay], $row['BlokID']);
                        break;
                    case $sat >= 20 && $sat <= 24:
                        $freeBlocksForDays[5][$availableDay][$key_pom]=$row['BlokID'];
...
//                                    and C.Vikend = $vikend";
                ...
                $price = number_format( ((100-$discount)/100) * $result1['Cena'] , 2);

                $price=$spotDuration*CENA;
                $price=number_format( ((100-$discount)/100) *$price , 2);
...
/*
                    echo $prvi_set_pocetak_i.' - '.$prvi_set_kraj_i.'<br>';
                    echo $drugi_set_pocetak_i.' - '.$drugi_set_kraj_i.'<br>';
                    exit;*/
