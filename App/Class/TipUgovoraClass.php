 ...
 * @author n.lekic
 */
class TipUgovoraClass {
    
    public function TipUgovoraGetForComboBox(TipUgovora $object) {
    ...
    public function TipUgovoraLoad(TipUgovora $tipUgovora) {
        $query = "select 
                    T.TipUgovoraID as tipUgovoraID,
                    T.Naziv as naziv,
                    case 
                    ...
                    where T.tipUgovoraID = ".$tipUgovora->getTipUgovoraID();
        $dbBroker = new CoreDBBroker();
    ...
    public function TipUgovoraInsert(TipUgovora $tipUgovora){
        $dbBroker = new CoreDBBroker();
        $dbBroker->beginTransaction();
        $result = $dbBroker->insert($tipUgovora);            
        $responseNew = new CoreAjaxResponseInfo();
        if ($result) {
            $dbBroker->commit();
            $responseNew->SetSuccess('true');
            $responseNew->SetMessage("Uspešno insertovan tip ugovora");
        }
        else {
            $dbBroker->rollback();
            $responseNew->SetSuccess('false');
